...
import type { CheckInGalleryItem, Show } from '@/lib/types/checkin';
...
export default function CheckInGallery({ showId, limit = 12 }: CheckInGalleryProps) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch shows for the filter dropdown
        ...
        // Apply show filter if selected (filter by show_date which corresponds to show.date)
        if (selectedShowId) {
        ...
        // Reset or append based on offset
        if (offset === 0) {
  ...
  // Reset offset when show filter changes
  useEffect(() => {
    setOffset(0);
    setHasMore(true);
  }, [selectedShowId]);
    ...
    <div className="space-y-6">
      ...
      {!showId && shows.length > 0 && (
        <div className="flex items-center gap-4 p-4 bg-zinc-800/50 rounded-lg border border-zinc-700/30">
          ...
          {selectedShowId && (
            <span className="text-xs text-zinc-400">({checkins.length} check-ins)</span>
          )}
        </div>
      )}
        ...
        >
          <div className="relative max-w-4xl max-h-full">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-6 rounded-b-lg">
              ...
              {lightboxImage.feedback && (
                <p className="text-zinc-300 mt-3 italic">"{lightboxImage.feedback}"</p>
              )}
            </div>
          </div>
        </div>
