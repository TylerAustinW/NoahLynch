...
import { SUPABASE_BUCKETS, SUPABASE_TABLES, FEATURE_FLAGS } from "@/lib/config/constants";
import type { CheckInSubmissionResponse } from "@/lib/types";

const supabaseAnonymous = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!);
...
export async function POST(request: NextRequest) {
    if (!FEATURE_FLAGS.CHECKIN_ENABLED) {
        return NextResponse.json<CheckInSubmissionResponse>(
            {
                success: false,
                message: "Check-in feature is currently disabled",
        ...
        for (const fieldName of mediaFields) {
            ...
            if (file && file.size > 0) {
                try {
                    ...
                    const { error: mediaInsertError } = await supabaseAnonymous.from(SUPABASE_TABLES.CHECKIN_MEDIA).insert({
